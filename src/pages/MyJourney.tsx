...
export const MyJourney = () => {
  ...
  const handleSaveProfile = async () => {
    ...
    const { error } = await supabase.from('profiles').update({
      full_name: fullName.trim(), bio, linkedin_url: linkedin, avatar_url: avatarUrl, theme_color: themeColor, interests, whatsapp, whatsapp_authorized: !!whatsapp,
    }).eq('id', user.id);

    if (error) toast.error('Erro ao salvar perfil');
    else { await fetchUserProfile(true); toast.success('Perfil atualizado!'); navigate('/feed'); }
  };
        ...
        <div className="mb-8">
          ...
          {journeyItems.map((item) => (
            <div key={item.id} className="bg-[#15335E] rounded-3xl p-6 mb-4 flex justify-between items-center">
              ...
              <div className="flex gap-3"><button onClick={() => { setNewItem(item); setEditingId(item.id); setIsAdding(true); }} className="text-blue-400"><Edit3 size={20} /></button><button onClick={() => handleDeleteItem(item.id)} className="text-red-400"><Trash2 size={20} /></button></div>
            </div>
          ))}
        </div>

        {/* BLOCO DE NOTIFICAÇÕES PUSH (Garantido que aparece) */}
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-slate-400 text-sm leading-relaxed max-w-lg">
                Ative as notificações para receber atualizações do sistema e novas oportunidades em tempo real, mesmo com o navegador fechado.
              </p>
            </div>
            <button
              onClick={subscribeToPushNotifications}
              className="w-full md:w-auto shrink-0 bg-[#142239] border border-[#D5205D] text-[#D5205D] hover:bg-[#D5205D] hover:text-white px-8 py-4 rounded-2xl font-bold transition-all duration-300 shadow-lg active:scale-95"
            >
              Ativar Alertas
            </button>
          </div>
