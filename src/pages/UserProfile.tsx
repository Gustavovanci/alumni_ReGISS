  ...
  return (
    <div className="min-h-screen bg-[#142239] text-slate-100 p-4 md:p-8 pb-24">
      <div className="max-w-4xl mx-auto">
        ...
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-slate-400 hover:text-white mb-6 font-medium"
        >
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              {profile.whatsapp && (
                <button
                  ...
                  className="mt-8 inline-flex items-center gap-3 bg-[#25D366] hover:bg-[#20bd5a] text-white px-8 py-4 rounded-3xl font-bold transition-all shadow-lg"
                >
          ...
          {!isCoordinator && profile.interests && profile.interests.length > 0 && (
            <div className="mt-8 flex flex-wrap gap-2">
              {profile.interests.map((tag: string, i: number) => (
                <span
                  key={i}
                  className="text-xs bg-white/5 text-slate-300 px-4 py-2 rounded-2xl border border-white/10"
                >
                  {tag}
                </span>
              ))}
